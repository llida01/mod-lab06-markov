...
std::vector <std::string> CreateSuffixTable(prefix Prefix, std::vector <std::string> text) {
...
std::map<prefix, std::vector<std::string>> CreatePrefixSuffixTable(std::deque <prefix> prefixTable, std::vector <std::string> text) {
    for (int i = 0; i < prefixTable.size(); i++) {
        std::vector <std::string> suffix = CreateSuffixTable(prefixTable[i], text);
...
std::string Generation(std::deque <prefix> prefixTable, int length) {
    ...
    std::map<prefix, std::vector<std::string>>::iterator it = statetab.find(nextPrefix);
